...
images = glob.glob("./images/labeled/*.png")
print(f"found {len(images)} original images")
new_dir = "./images/augmented/"
try:
    os.mkdir(new_dir)
...
for image_file in images:
    ...
    x = 2
    for i in range(3):
        new_img = rotate_image(img, random.randint(-20, 20))
        cv2.imwrite(f"{new_dir}{img_name}-{str(x)}.png", new_img)
        x += 1

        new_img = rotate_image(img_flip, random.randint(-20, 20))
